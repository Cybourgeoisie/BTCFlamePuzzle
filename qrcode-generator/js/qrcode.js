...
var qrcode = function() {
  ...
  var qrcode = function(typeNumber, errorCorrectionLevel) {
    ...
    // Debugging and mucking around
    var _dIncludeModeAndCharLengthDatabits = true;
    var _dIncludeDataBits = true;
    var _dIncludeErrorCorrection = true;
    var _dApplyMaskPattern = true;
    var _dRecordChangesToDataBits = true;
    var _orderOfDataBits = [];
    var _dataBitsChanged = [];
    var _maskPatternOverride = null;

    _this.dSetModeCharLength = function(s) {
      _dIncludeModeAndCharLengthDatabits = !!s;
    }

    _this.dSetAddDataBits = function(s) {
      _dIncludeDataBits = !!s;
    }

    _this.dSetAddErrorCorrectionBits = function(s) {
      _dIncludeErrorCorrection = !!s;
    }

    _this.dSetApplyMaskPattern = function(s) {
      _dApplyMaskPattern = !!s;
    }

    _this.setMaskPattern = function(mp) {
      if (mp < 0 || mp > 7)
        throw "Invalid mask pattern: " + mp;
      _maskPatternOverride = mp;
    }

    _this.dSetRecordChangesToDataBits = function(s) {
      _dRecordChangesToDataBits = !!s;
    }

    _this.dOnlyPrintImageSrc = function(s) {
      _dOnlyImageSrc = !!s;
    }
    ...
    _this.getOrderOfDataBits = function() {
      return _orderOfDataBits;
    }

    _this.getDataBitsChanged = function() {
      return _dataBitsChanged;
    }
    ...
    var makeImpl = function(test, maskPattern) {
      ...
      // Add the three corner probes
      setupPositionProbePattern(0, 0);
      setupPositionProbePattern(_moduleCount - 7, 0);
      setupPositionProbePattern(0, _moduleCount - 7);

      // Add the position adjustment probes inside the QR code
      setupPositionAdjustPattern();

      // Add the timing patterns along the left and top
      setupTimingPattern();

      // Add the type information for the mask pattern
      setupTypeInfo(test, maskPattern);
      if (_typeNumber >= 7) {
        setupTypeNumber(test);
      }

      // Now throw the data and error correcting blocks in
      // It looks possible to pass in enough data to fill the rest of the data blocks
      // Not sure about the beginning, though - the mode bits and the character count bits
      if (_dataCache == null) {
        _dataCache = createData(_typeNumber, _errorCorrectionLevel, _dataList);
      }

      // Finally, map the data and apply the mask pattern
      mapData(_dataCache, maskPattern, test);
    };
    ...
    var setupTypeInfo = function(test, maskPattern) {
      ...
      //var data = (_dApplyMaskPattern) ? (_errorCorrectionLevel << 3) | maskPattern : (_errorCorrectionLevel << 3);
    ...
    var mapData = function(data, maskPattern, test) {
      ...
      /*
      var hexed_data = []
      for (var i = 0; i < 152/2; i++) {
        hexed_data.push(("00" + data[i].toString(16)).substr(-2));
      }
      console.log(hexed_data.join(''));
      */
        ...
        while (true) {
          ...
          for (var c = 0; c < 2; c += 1) {
            ...
            if (_modules[row][col - c] == null) {
              ...
              if (!test)
                _orderOfDataBits.push([row, col - c]);
              ...
              if (mask && _dApplyMaskPattern) {
                dark = !dark;
              }

              if (!test && _dRecordChangesToDataBits)
                _dataBitsChanged.push(dark ? 1 : 0);
    ...
    var createData = function(typeNumber, errorCorrectionLevel, dataList) {
      ...
      // Get the RS information wrt the type number and desired EC level
      var rsBlocks = QRRSBlock.getRSBlocks(typeNumber, errorCorrectionLevel);

      var buffer = qrBitBuffer();

      // This is peculiar. Why add the mode and type number for each set of data
      // in the data list? Unless we're only providing one element in the data list..
      // Looks like, yes, that's the idea.
      for (var i = 0; i < dataList.length; i += 1) {
        var data = dataList[i];

        if (_dIncludeModeAndCharLengthDatabits) {
  ...
  var qrRawHex = function(data) {

    var _data = data;

    var _bytes = [];
    for (var i = 0; i < data.length; i += 2) {
      _bytes.push(data.charAt(i) + '' + data.charAt(i+1));
    }

    var _this = {};

    _this.getMode = function() {
      throw "Not possible for qrRawHex";
    };
    ...
    _this.write = function(buffer) {
      for (var i = 0; i < _bytes.length; i += 1) {
        buffer.put(parseInt(_bytes[i], 16), 8);
      }
    };
